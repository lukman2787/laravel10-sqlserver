...
class CostAnalysisController extends Controller
{
    public function index(request $request)
    {
        ...
        return view('productions.cost_analysis.index', ['tree' => $tree]);
    }

    // function buildHierarchy($items, $parentId = null)
    // {
    //     $hierarchy = [];
    //     $items = $this->get_bom_price();

    //     foreach ($items as $item) {
    //         if ($item->Father == $parentId) {
    ...
    function generateLaravelSyntax($data)
    {
        $syntax = "";

        foreach ($data as $node) {
            $syntax .= $this->generateLaravelSyntaxRecursive($node);
        }
    ...
    function generateLaravelSyntaxRecursive($node)
    {
        $syntax = "";

        $syntax .= "nodes::create([";
        $nodeArray = (array) $node; // Convert object to array
        ...
        if (!empty($nodeArray['Depth'])) {
            foreach ($nodeArray['Depth'] as $child) {
                $syntax .= $this->generateLaravelSyntaxRecursive($child); // Removed "$this->"
            }
        }
    ...
    public function get_parent_bom(Request $request)
    {
        ...
        foreach ($tree as $key => $item) {
            $jml_line = mb_strlen($item->lineage);
            $tree_line = $jml_line . $item->BOMType;
            if ($item->Depth == 2) {
                if ($item->BOMType == 'N') {
                    $output .= '
                                            <tr>
                                                <td>' . $item->lineage . ' ' . $item->Item . ' ' . $item->ItemDescription . '</td>
                                                <td>' . number_format($item->qty, 2) . ' ' . $item->UoM . '</td>
                                                <td>' . $item->Curr . ' ' . number_format($item->Price, 0) . '</td>
                                                <td>' . $item->Curr . ' ' . number_format($item->qty * $item->Price, 0) . '</td>
                                            </tr>';
                } else if ($item->BOMType == 'P') {
                    $output .= '
                                                    ...
                                                    <td>
                                                        <div class="p-0">
                                                            <table class="table table-hover">
                                                                <tbody>
                                                                    <tr data-widget="expandable-table" aria-expanded="true">';
                            return view('partials.tree_item_first', ['item' => $child]);
                            $output .= '</tr>
                                                                </tbody>
                                                            </table>
                                                        </div>
